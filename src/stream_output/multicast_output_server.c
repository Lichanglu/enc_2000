...
/*********************************static*****function****end*******************************************/
extern int app_get_encode_rate(int channel, int *vrate, int *arate , int *quality);
...
static int multicast_node_config(stream_output_server_config *config, MULTICAST_Output_Server_NODE *node)
{
	...
	app_get_encode_rate(stream_channel, &vrate, &arate, &vcbr);
...
static int app_mult_set_node_tc(MULTICAST_Output_Server_NODE *node)
{
	...
	app_get_encode_rate(node->config.stream_channel, &vrate, &arate, &vcbr);
...
{
	PRINTF("i will set the %d channel tc\n", channel);
	...
	while(node != NULL) {
		if(node->config.stream_channel == channel) {
			PRINTF("i will set tc \n");
...
#endif
static int g_ts_mtu[STREAM_MAX_CHANNEL] = {1450, 1450, 1450, 1450};
static int g_rtp_mtu[STREAM_MAX_CHANNEL] = {1450, 1450, 1450, 1450};
static int g_rtpts_mtu[STREAM_MAX_CHANNEL] = {1450, 1450, 1450, 1450};
...
int stream_get_rate(int cmd, int channel)
{
	int vrate = 0;
	int arate = 0;
	int vcbr = 0;
	...
	app_get_encode_rate(channel, &vrate, &arate, &vcbr);

	PRINTF("channel=%d,vrate=%d,arate=%d,vcbr=%d\n", channel, vrate, arate, vcbr);

	if(cmd == MSG_MULT_GET_TS_RATE) {
		type = TYPE_TS;
	} else if(cmd == MSG_MULT_GET_RTP_RATE) {
		type = TYPE_RTP;
	} else if(cmd == MSG_MULT_GET_RTPTS_RATE) {
		type = TYPE_TS_OVER_RTP;
	} else {
		type = TYPE_RTP;
	}

	if(vcbr == 1) {
		total_rate = app_trans_rate(vrate, arate, type);
