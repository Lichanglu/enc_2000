...
int app_build_rtp_vdata(int len, unsigned char *buff, int IsIframe, int mtu, unsigned int rtp_time, APP_MEDIA_INFO *media_info)
{
	...
	//PRINTF("g_video_rtp_handle[%d] = %p\n", id, g_video_rtp_handle[id]);
	rtp_build_video_data(g_video_rtp_handle[id], len, buff, IsIframe, mtu, rtp_time, media_info);
...
int app_build_rtp_adata(int len, unsigned char *buff, int samplerate, int mtu, unsigned int rtp_time, APP_MEDIA_INFO *media_info)
{
	...
	//PRINTF("g_audio_rtp_handle[%d] = %p,buflen=%d\n", id, g_audio_rtp_handle[id],len);
	rtp_build_audio_data(g_audio_rtp_handle[id], len, buff, samplerate, mtu, rtp_time, media_info);
