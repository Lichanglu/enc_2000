...
static int stream_rtsp_set_clinet_tc(int channel, int num, int tc_flag, int tc_rate, char *dst_ip, unsigned short port);
...
int app_rtsp_server_set_common_info(stream_output_server_config *in, stream_output_server_config *out)
{
	...
	if(tc_change == 1) {
		...
		for(x = CHANNEL_INPUT_1 ; x < STREAM_MAX_CHANNEL; x++) {
			stream_rtsp_set_all_tc(x);
		}
	}
...
int stream_server_config_printf(stream_output_server_config *config)
{
	...
	PRINTF("server config %p,stream_channel %d ,the conf nume=%d,type=%d,status=%d\n", config, config->stream_channel, config->num, config->type, config->status);
...
==============================================================================*/
int stream_get_rtsp_ginfo(int *mult, char *ip, unsigned short *vport, unsigned short *aport, int stream_channel)
{
	if(g_rtsp_shandle == NULL) {
		PRINTF("\n");
		return -1;
...
void *stream_create_rtsp_client(int pos, struct sockaddr_in *video_addr, struct sockaddr_in *audio_addr, int stream_channel)
{
	...
	stream_rtsp_set_clinet_tc(stream_channel, num, tc_flag, tc_rate, dst_ip,  port);
...
int stream_rtsp_set_all_tc(int channel)
{
	...
	for(i = 0 ; i < RTSP_MAX_CLIENT_NUM ; i++) {
		...
		if(client == NULL || client->c_used == NOTUSED || client->stream_channel != channel) {
			continue;
		}
		...
		PRINTF("num = %d,tc_flag =%d,tc_Rate=%d,dsp_ip=%s,port=%u.\n", num, tc_flag, tc_rate, dst_ip, port);
		stream_rtsp_set_clinet_tc(client->stream_channel, num, tc_flag, tc_rate, dst_ip,  port);
	}
...
static int stream_rtsp_set_clinet_tc(int stream_channel, int num, int tc_flag, int tc_rate, char *dst_ip, unsigned short port)
{
	int vrate = 0;
	int vcbr = 0;
	int arate = 0;
