...
#define MMP_IND_INPUT SIGNAL_INPUT_1
#define MMP_IND_HIGH_CHANNEL CHANNEL_INPUT_1
#define MMP_IND_LOW_CHANNEL CHANNEL_INPUT_1_LOW
...
#define MMP_SYSINFO_INDEX   MAIN_MMP
...
static void print_client_npos(char *buf);
...
static int app_mmp_vinfo_set(int chid, unsigned char *data)
{
	...
	if(change == 1) {
		...
		if(chid  == CHANNEL_INPUT_MP) { //mp ->input1
			MMP_config_update2(HIGH_STREAM, 2 , (unsigned char *)&old_param, sizeof(MMPVideoParam));
		}
	}
	...
	for(cnt = 0; cnt < MAX_CLIENT; cnt++) {
		if(ISUSED(PORT_ONE, cnt) && ISLOGIN(PORT_ONE, cnt))	{
			...
			if(ret < 0) {
				...
				//				print_client_npos("SendBroadCastMsg");
			}
		}
	}
	...
	for(cnt = 0; cnt < MAX_CLIENT; cnt++) {
		if(ISUSED(PORT_ONE, cnt) && ISLOGIN(PORT_ONE, cnt) && GETLOWRATEFLAG(PORT_ONE, cnt) == LOW_VIDEO_START)	{
			...
			if(ret < 0) {
				ERR_PRN("SendBroadCastMsg()return:%d socket:%d ERROR:%d\n", ret, GETSOCK(PORT_ONE, cnt), errno);
				SETCLIUSED(PORT_ONE, cnt, FALSE);
				//				print_client_npos("LowRateBroadCastMsg");
				SETCLILOGIN(PORT_ONE, cnt, FALSE);
			}
		}
	}
...
static void EncoderProcess(void *pParams)
{
	...
	while(gRunStatus) {
		...
		switch(phead->nMsg) {
			...
			case MSG_FARCTRL:
				PRINTF("MSG_FARCTRL\n");
				FarCtrlCamera(DSP1, (unsigned char *)&szData[HEAD_LEN], nMsgLen - HEAD_LEN, input_get_remote(0));
				break;
			...
			case MSG_PASSWORD:
				PRINTF("MSG_PASSWORD\n");
				{
					...
					//					print_client_npos("EncoderProcess");
...
ExitClientMsg:
	...
	//	print_client_npos("EncoderProcess_1");
...
static void InitClientData(unsigned char port_num)
{
	...
	for(cli = 0; cli < MAX_CLIENT; cli++) 	{
		...
		//		print_client_npos("InitClientData");
...
SERVERSTARTRUN:
	...
	ret1 = sys_get_network(ETH0, &ip, &gateway, &netmask);
	...
	while(gRunStatus) {
		...
		if(sClientSocket > 0) {
			...
			int nPos = 0;
			//			print_client_npos("EncoderServerThread_1");
			...
			if(-1 == nPos) {
				...
				//				print_client_npos("EncoderServerThread_2");
...
void SendAudioToClient1(int nLen, unsigned char *pData,
			...
			if(ISUSED(index, cnt) && ISLOGIN(index, cnt)
			   && (GETLOWRATEFLAG(0, cnt) != LOW_VIDEO_START)) {
				...
				if(nRet < 0) {
					...
					//					print_client_npos("SendAudioToClient1");
			...
			if(ISUSED(index, cnt) && ISLOGIN(index, cnt)
			   && (GETLOWRATEFLAG(0, cnt) == LOW_VIDEO_START)) {
				...
				if(nRet < 0) {
					...
					//					print_client_npos("LowSendAudioToClient1");
					SETCLILOGIN(index, cnt, FALSE);
					PRINTF("Audio Send error index:%d, cnt:%d %d nSendLen:%d  ,nRet:%d\n", index, cnt, errno, nSendLen, nRet);
				}
			}
...
void SendDataToClient1(int nLen, unsigned char *pData,
                       int nFlag, unsigned char index, int width, int height)
{
	...
	DataFrame.nTimeTick = timeTick;
	//printf("video[1]:[%d] pts:[%d]\n", timeTick, (timeTick-tmpTick));
	//	tmpTick = timeTick;
			...
			if(ISUSED(0, cnt) && ISLOGIN(0, cnt) && (GETLOWRATEFLAG(0, cnt) != LOW_VIDEO_START)) {
				...
				if(sendsocket > 0) {
					...
					if(nRet < 0) {
						...
						//						print_client_npos("SendDataToClient1");
...
void LowSendDataToClient1(int nLen, unsigned char *pData,
                          int nFlag, unsigned char index, int width, int height)
{
			...
			if(ISUSED(0, cnt) && ISLOGIN(0, cnt) && (GETLOWRATEFLAG(0, cnt) == LOW_VIDEO_START)) {
				...
				if(sendsocket > 0) {
					...
					if(nRet < 0) {
						...
						//						print_client_npos("LowSendDataToClient1");
						SETCLILOGIN(0, cnt, FALSE);
						//		SETPPTQUERY(0, cnt, FALSE);
						PRINTF("Error: SOCK = %d count = %d  errno = %d  ret = %d\n", sendsocket, cnt, errno, nRet);
					}
