...
int app_get_encode_rate(int channel, int *vrate, int *arate , int *quality)
{


	*arate = app_audio_get_rate(channel) / 1000;
	app_video_get_rate(channel, vrate, quality);

	PRINTF("channel  =%d,vrate=%d,arate=%d,quality=%d\n", channel, *vrate, *arate, *quality);
	return 0;
}
